...
def main(cfg):
    ...
    classes = sorted(os.listdir(os.path.dirname(ShapeNetCoreV2.__file__)))
    for idx, cls in enumerate(classes):
        cls_int2str_dict[idx] = cls
        cls_str2int_dict[cls] = idx
    ...
    with Pool(processes=cpu_count()) as pool:
        for result in pool.imap_unordered(process_single_mesh, args_list):
            ...
            samples_dict[obj_idx] = {
                'sdf': result['sdf'],
                'samples_latent_class': result['samples_latent_class']
            }
            idx_str2int_dict[result['idx_str']] = obj_idx
            idx_int2str_dict[obj_idx] = result['idx_str']
    ...
    np.save(os.path.join(out_dir, 'cls_str2int_dict.npy'), cls_str2int_dict)
    np.save(os.path.join(out_dir, 'cls_int2str_dict.npy'), cls_int2str_dict)
