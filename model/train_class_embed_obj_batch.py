...
class Trainer():
    ...
    def __call__(self):
        ...
        self.class_embeds = torch.nn.Embedding(len(train_dataset.classes), self.train_cfg['class_embed_size'], max_norm=1.0).to(device)
        ...
        for epoch in range(self.train_cfg['epochs']):
            ...
            if avg_val_loss < best_loss:
                ...
                best_class_embeds = self.class_embeds.weight.detach().cpu().numpy()
    ...
    def train(self, batch):
        ...
        batch_class_embeds = self.class_embeds(batch['class_id']).unsqueeze(1).expand(-1, batch['points_train'].shape[1], -1)
        batch_class_embeds = batch_class_embeds.reshape((num_points_per_batch, self.train_cfg["class_embed_size"]))

        # reshape points and sdf for forward pass
        points = batch['points_train'].reshape((num_points_per_batch, 3))
        
        x = torch.concat((batch_class_embeds, batch_latent_vectors,points), dim=1)
        ...
        loss_value, loss_rec, loss_latent = self.train_cfg['loss_multiplier'] * SDFLoss_multishape(y, 
                                                                                                   predictions, x[:, :(self.train_cfg['latent_size'] + self.train_cfg['class_embed_size'])], 
    ...
    def validate(self, batch):
        ...
        batch_class_embeds = self.class_embeds(batch['class_id']).unsqueeze(1).expand(-1, batch['points_val'].shape[1], -1)
        batch_class_embeds = batch_class_embeds.reshape((num_points_per_batch, self.train_cfg["class_embed_size"]))
