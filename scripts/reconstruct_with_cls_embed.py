...
def main(cfg):
    ...
    latent_size = training_settings['latent_size'] + training_settings["class_embed_size"]
    ...
    cls_str2int_dict = np.load(cls_str2int_path, allow_pickle=True).item()
    results_dict = np.load(results_dict_path, allow_pickle=True).item()

    # Warning: class embedding mod relies on objects constructed ONE by ONE
    # one after the other
    # so that all points have the same class embedding (because came from 1 object)
    ...
    for obj_id_path in cfg['obj_ids']:
        ...
        obj_cls = obj_id_path.split("/")[0]
        obj_cls = cls_str2int_dict[obj_cls]
        ...
        class_embed = results_dict['best_class_embeds'][obj_cls]
        class_embed = torch.tensor(class_embed).to(device)

        latent_vect = torch.hstack((class_embed, latent_code))

        reconstruct_object(cfg, latent_vect, obj_idx, model, coords_batches, grad_size_axis)
